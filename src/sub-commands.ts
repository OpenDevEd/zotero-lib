...
customCmdHandlers.set('ddresolve', 'resolvefunc');
customCmdHandlers.set('deduplicate', 'deduplicate_func');
customCmdHandlers.set('ddmovetocollection', 'Move_deduplicate_to_collection');
customCmdHandlers.set('ddmerge', 'merge_func');
...
subParsersMap.set('db', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Local database: Manage locally synced version of your library.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });
  argparser.add_argument('database', {
    action: 'store',
    help: 'This option allows you to create a local database of one or more groups.',
  });
...
subParsersMap.set('dbresolve', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Local database: Resolve a Zotero Select link (zotero://...) to a key.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });
  argparser.add_argument('--file', {
    action: 'store',
    help: 'The database path. For this command to work, you need to create a local database with the `db` command first.',
  });
  argparser.add_argument('--groupid', {
    action: 'store',
    help: 'The group id',
...
subParsersMap.set('deduplicate', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Deduplicate: Deduplicate items in a collection, generates a `duplicates.json` file with the results.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });
...
subParsersMap.set('ddmovetocollection', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Deduplicate: Move items from deduplicate json file to provided collection.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });
...
subParsersMap.set('ddmerge', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Deduplicate: Merge items from a specified JSON file.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });

  argparser.add_argument('--data', {
    action: 'store',
    help: 'The data for the merge needs to be provided as a json file. The file needs to be generated with the deduplicate command',
    required: true,
  });
...
subParsersMap.set('find-empty-items', function (subparsers, subCmdName) {
  const argparser = subparsers.add_parser(subCmdName, {
    help: 'Deduplicate: Finds empty items from the database.',
  });
  argparser.set_defaults({ func: getFuncName(subCmdName) });
