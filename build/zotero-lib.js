...
module.exports = class Zotero {
    constructor(args) {
        ...
        this.config_keys = ["user-id", "group-id", "library-type", "api-key", "indent", "verbose", "debug", "config", "config-json"];
      ...
      Also see 'attachment', 'create' and 'update'.
        */
        ...
        if (args.getInterface && subparsers) {
            ...
            parser_item.add_argument('--addfile      const zotero = new Zotero({ group_id: result.data.zoteroGroup }), s, ', { nargs: ' * ', help: ', Upload, attachments, to, the, item.(/items/new), ' }), parser_item.add_argument('--savefiles', { nargs: '*', help: 'Download all attachments from the item (/items/KEY/file).' }), parser_item.add_argument('--addtocollection', { nargs: '*', help: 'Add item to collections. (Convenience method: patch item->data->collections.)' }), parser_item.add_argument('--removefromcollection', { nargs: '*', help: 'Remove item from collections. (Convenience method: patch item->data->collections.)' }), parser_item.add_argument('--addtags', { nargs: '*', help: 'Add tags to item. (Convenience method: patch item->data->tags.)' }), parser_item.add_argument('--removetags', { nargs: '*', help: 'Remove tags from item. (Convenience method: patch item->data->tags.)' }));
            return { status: 0, message: "success" };
        }
        let output = [];
